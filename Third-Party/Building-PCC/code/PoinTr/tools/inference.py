...
def get_args():
    ...
    parser.add_argument('--save_predict', default=False, action="store_true", help='whether to save ply of predict point cloud')
    parser.add_argument('--save_merged',  default=False, action="store_true", help='whether to save merged point cloud (merge predict and input)')
    parser.add_argument('--save_vis_img',  default=False, action="store_true", help='Default not saving the visualization images.')
    parser.add_argument('--copy_original_files', default=False, action="store_true", help='whether to copy original point cloud, ground truth and 3D model to the inference folder')
    parser.add_argument('--evaluation_only', default=False, action="store_true", help='Perform evaluation on predicted data.')
    parser.add_argument('--with_recompute', default=False, action="store_true", help='Perform evaluation and recompute chamfer distance, only valid when evaluation_only is true.')
    parser.add_argument('--no_subdirs',  default=False, action="store_true", help='Do not create a separate directory for each file.)')
    ...
    #print(f'args.pc_root={args.pc_root}')
    #print(f'args.save_merged={args.save_merged}')
    #print(f'args.save_predict={args.save_predict}')
...
def inference_single(model, pc_path, args, model_config, avg_f_score, avg_chamfer_L1, avg_chamfer_L2, root=None):
    ...
    try:
        ...
        print(f'Cannot read file: {pc_file}')
        print(traceback.format_exc())
        print(e)

    if args.truth_root != '':
        #truth_file = os.path.join(args.truth_root, pc_path.split('.')[0] + ".ply")
        truth_file = os.path.join(args.truth_root, pc_path.split('.')[0] + ".npy")
    ...
    centroid = 0
    m = 0
    print(f'Model base name: {model_config.dataset.train._base_["NAME"]}')
    ...
    elif model_config.dataset.train._base_['NAME'] == 'PCN':
        pc_ndarray_trans = pc_ndarray

    #print(f'(inference_kernel start)')
    dense_points = inference_kernel(model, pc_ndarray_trans, args, pc_path)
    #print(f'(inference_kernel end)')

    if args.out_pc_root != '':
        if not args.no_subdirs:
            target_path = os.path.join(args.out_pc_root, os.path.splitext(pc_path)[0])
        else:
            target_path = os.path.join(args.out_pc_root)
    ...
    if args.save_predict:
        print(f'Saving predicted point cloud.')
        try:
            #IO._write_ply(os.path.join(target_path, pc_path.split('.')[0] + '_predict.ply'), dense_points)
            IO._write_xyz(os.path.join(target_path, pc_path.split('.')[0] + '_predict.xyz'), dense_points)
        except Exception as e:
            print(traceback.format_exc())
            print(e)
    ...
    if args.save_merged:
        ...
        print(f'Saving merged point cloud.')
        try:
            #IO._write_ply(os.path.join(target_path, pc_path.split('.')[0] + '_merged.ply'), merged_points)
            IO._write_xyz(os.path.join(target_path, pc_path.split('.')[0] + '_merged.xyz'), merged_points)
        except Exception as e:
            print(traceback.format_exc())
            print(e)
    ...
    if args.copy_original_files:
        ...
        pc_orig_target_path = os.path.join(target_path, split_path[0] + "_partial." + split_path[1])
        shutil.copy(pc_file, pc_orig_target_path)

        if args.truth_root != '':
            #truth_target_path = os.path.join(target_path, pc_path.split('.')[0] + "_complete.ply")
            truth_target_path = os.path.join(target_path, pc_path.split('.')[0] + "_complete.xyz")
        ...
        if args.model_3d_root != '':
            ...
            model_3d_target_path = os.path.join(target_path, pc_path.split('.')[0] + "_model.xyz")
    ...
    if args.evaluation_only:
        ...
        if args.pc_root != '':
            ...
            for pc_file in tqdm(pc_file_list, desc="Processing Files"):
                print(f' Processing {os.path.splitext(pc_file)[0]}...')
                target_path = os.path.join(args.out_pc_root, os.path.splitext(pc_file)[0])
                eval_path = os.path.join(target_path, pc_file.split('.')[0] + "_eval.txt")
                pred_path = os.path.join(target_path, pc_file.split('.')[0] + '_predict.ply')
                ...
                if os.path.exists(eval_path) and os.path.exists(pred_path) and os.path.exists(comple_path):
                    ...
                    try:
                        ...
                        print(f'Could not inference file: {pc_file}')
                        print(traceback.format_exc())
                        print(e)
