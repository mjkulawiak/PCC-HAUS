...
def inference_single(model, pc_path, args, config, root=None):
    ...
    if args.out_pc_root != '':
        ...
        target_path_npy = os.path.join(target_path, 'npy')
        target_path_xyz = os.path.join(target_path, 'xyz')
        os.makedirs(target_path_npy, exist_ok=True)
        os.makedirs(target_path_xyz, exist_ok=True)
        #np.save(os.path.join(target_path, 'fine.npy'), dense_points)
        np.save(os.path.join(target_path_npy, pc_path.split('.')[0] + '_predict.npy'), dense_points)
        try:
            #IO._write_ply(os.path.join(target_path, pc_path.split('.')[0] + '_predict.ply'), dense_points)
            IO._write_xyz(os.path.join(target_path_xyz, pc_path.split('.')[0] + '_predict.xyz'), dense_points)
