...
def test_net(cfg, epoch_idx=-1, test_data_loader=None, test_writer=None, model=None, generate_partial=False):
                ...
                with torch.no_grad():
                    ...
                    for partial_id, item in enumerate(choice):
                        ...
                        if partial.shape[1] < 2048:
                            raise ValueError(
                                f'Not enough points for FPS subsampling: {partial.shape[1]} < 2048')
                        partial = fps_subsample(partial, 2048)
                        #print(f'pt[{partial_id}] = {partial} ({partial.size()})')
                    ...
                    for mm, model_name in enumerate(model_ids):
                        ...
                        try:
                            ...
                            Path(img_filename).parent.mkdir(parents=True, exist_ok=True)
