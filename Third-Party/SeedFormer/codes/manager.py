...
class Manager:
    ...
    def unpack_data(self, data, ignore_truth=False):
        if self.dataset == 'ShapeNet' or self.dataset == 'ShapeNet55':
    ...
    def test(self, cfg, model, test_data_loader, outdir, mode=None, generate_partial=False, ignore_truth=False):

        if self.dataset == 'ShapeNet':
            self.test_pcn(cfg, model, test_data_loader, outdir)
        elif self.dataset == 'ShapeNet55':
            self.test_shapenet55(cfg, model, test_data_loader, outdir, mode, generate_partial, ignore_truth)
        else:
    ...
    def test_shapenet55(self, cfg, model=None, test_data_loader=None, outdir=None, mode=None, generate_partial=False, ignore_truth=False):
        """
        ...
        # Start testing
        print(f'Start evaluating (mode: {mode}, generate_partial: {generate_partial}) ...')
        for model_idx, (taxonomy_id, model_id, data) in enumerate(test_data_loader):
            print(f'Processing {model_id[0]} ({model_idx})')
            try:
                taxonomy_id = taxonomy_id[0] if isinstance(taxonomy_id[0], str) else taxonomy_id[0].item()
                #model_id = model_id[0]
                ...
                with torch.no_grad():
                    ...
                    if (not generate_partial):
                        # unpack data
                        print(f'Unpacking partial data.')
                        partial, gt = self.unpack_data(data, ignore_truth)
                        #partial = fps_subsample(partial, 2048)
                    else:
                        # generate partial data online
                        print(f'Generating partial data from ground-truth data.')
                        gt = data['gtcloud']
                    ...
                    if (gt is not None):
                        ...
                        print(f'Skipping metrics calculation due to lack of ground-truth data.')
